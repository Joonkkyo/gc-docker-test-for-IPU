...
def get_norm_layer(args):
    if args['norm_type'] == "none":
        ...
        return partial(torch.nn.BatchNorm2d, momentum=args['batchnorm_momentum'], eps=float(args['norm_eps']))
    elif args['norm_type'] == "group":
        return partial(torch.nn.GroupNorm, args['norm_num_groups'], eps=float(args['norm_eps']))
